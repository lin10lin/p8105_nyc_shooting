...
```
## Add city council distric 
```{r}
df <- read_csv("data_neighborhood_date_time_population_cdta.csv")
...
city_council_sf <- st_read("city_council_districts.geojson")

if (st_crs(df_sf) != st_crs(city_council_sf)) {
  city_council_sf <- st_transform(city_council_sf, st_crs(df_sf))
}

# Perform spatial join using the correct column name
joined_sf <- st_join(df_sf, city_council_sf["coun_dist"], left = TRUE)
...
write_csv(joined_df, "data_with_city_council_districts.csv")
